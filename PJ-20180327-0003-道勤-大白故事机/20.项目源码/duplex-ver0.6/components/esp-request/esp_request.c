...
static int req_process_upload(request_t *req)
{
    ...
    if (req->upload_data_ptr && req->upload_data_len > 0) {
        int bwr = req->_write(req, req->upload_data_ptr, req->upload_data_len);
        ESP_LOGD(REQ_TAG, "end write %d bytes", bwr);
        if(bwr < 0) {
            ESP_LOGE(REQ_TAG, "Error write");
            return -1;
        }
