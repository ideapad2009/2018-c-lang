		...
		switch (status)
		{
			case MPUSH_STAUTS_INIT:
			{
				...
				if (_mpush_client_get_device_sn(pHandler) == MPUSH_ERROR_GET_SN_OK)
				{
					...
					mpush_set_run_status(pHandler, MPUSH_STAUTS_GET_ACOUNT_INFO);
					//to do 增加语音提示，激活成功
					player_mdware_play_tone(FLASH_MUSIC_B_20_DEVICE_ACTIVATE_OK);
					vTaskDelay(2000);
				}
				else
				{
					ESP_LOGE(PRINT_TAG, "_mpush_client_get_device_sn fail");
					player_mdware_play_tone(FLASH_MUSIC_B_19_DEVICE_ACTIVATE_FAIL);
					device_sleep(10, 0);
				}
				break;
			}
	...
	const char *_to_user)
{
	...
	MPUSH_CLIENT_SEND_MSG_BUFFER *buffer = &_pHandler->send_msg_buffer;
	memset(buffer, 0, sizeof(MPUSH_CLIENT_SEND_MSG_BUFFER));
	
	if (sock_get_server_info(MPUSH_SEND_MSG_URL, &buffer->domain, &buffer->port, &buffer->params) != 0)
	{
	...
	{
		snprintf(buffer->str_type, sizeof(buffer->str_type), "file");
	}
	else if (_msg_type == MPUSH_SEND_MSG_TYPE_TEXT)
	{
		snprintf(buffer->str_type, sizeof(buffer->str_type), "text");
	}
	else
	{
	...
	snprintf(buffer->str_buf, sizeof(buffer->str_buf), 
		...
		"Key: %s\r\n"
		"Connection:close\r\n\r\n", 
		buffer->params, buffer->domain, buffer->port, strlen(_pHandler->str_comm_buf), buffer->str_nonce, buffer->str_timestamp, buffer->str_private_key, DEEP_BRAIN_ROBOT_ID);
