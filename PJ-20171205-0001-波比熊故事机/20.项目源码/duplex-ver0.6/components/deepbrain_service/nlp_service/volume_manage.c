	...
	switch (volume_control_state)
	{
		case VOLUME_CONTROL_HIGHER:
		{
			...
			if (need_to_set_vol > 100)
			{
				...
				ESP_LOGI(VOLUME_TAG, "after set vol = %d (line %d)", 100, __LINE__);
		...
		case VOLUME_CONTROL_LOWER:
		{
			...
			if (need_to_set_vol < 0)
			{
				...
				ESP_LOGI(VOLUME_TAG, "after set vol = %d (line %d)", 0, __LINE__);
		...
		case VOLUME_CONTROL_MODERATE:
		{
			...
			ESP_LOGI(VOLUME_TAG, "after set vol = %d (line %d)", 55, __LINE__);
