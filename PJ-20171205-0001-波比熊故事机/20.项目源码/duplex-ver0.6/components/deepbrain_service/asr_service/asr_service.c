...
static void record_encode_create(ASR_SERVICE_HANDLE_T *asr_service_handle, ASR_RECORD_TYPE_T record_type)
{
	switch (record_type)
	{
...
static void record_asr_process(ASR_SERVICE_HANDLE_T *asr_service_handle, ASR_RECORD_MSG_T *p_msg)
{
	...
	switch (p_msg->record_data.record_type)
	{
		case ASR_RECORD_TYPE_AMRNB:
		{
			...
			if (baidu_asr_amrnb(&asr_service_handle->baidu_acount, p_asr_result->asr_text, sizeof(p_asr_result->asr_text), 
				(unsigned char*)p_asr_result->record_data, p_asr_result->record_len, BAIDU_ASR_MODE_1536) == BAIDU_ASR_FAIL)
			{
	...
	ESP_LOGI(LOG_TAG, "asr result:record_len[%d],cost[%lldms],result[%s]", p_asr_result->record_len, asr_cost, p_asr_result->asr_text);
	...
	{		
		...
		{			
			switch (p_msg->msg_type)
			{
				...
				{
					//创建音频编码句柄
					record_encode_create(asr_service_handle, p_msg->record_data.record_type);
#if DEBUG_RECORD_PCM
