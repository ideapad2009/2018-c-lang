...
#define SD_PLAYER_CTRL_SER_TAG	"sd music manage"
...
static void task_scan_sd_music(void *pv)
{
    ...
    while (1) 
	{		
		if (xQueueReceive(sd_music_manage->x_queue_sd_mount, &msg, portMAX_DELAY) == pdPASS) 
		{
			switch (msg)
			{
				case SD_MUSIC_MSG_MOUNT:
				{
					player_mdware_play_tone(FLASH_MUSIC_B_12_SD_CARD_HAS_BEEN_INSERTED);
				...
				{
					player_mdware_play_tone(FLASH_MUSIC_B_13_SD_CARD_HAS_BEEN_REMOVED);
					break;
				}
				default:
					break;				
...
void sd_music_manage_create(void)
{
	...
	g_sd_music_handle->x_queue_sd_mount = xQueueCreate(2, sizeof(int));
	...
	/*
    if (xTaskCreate(_task_sd_player_ctrl,
                    "_task_sd_player_ctrl",
                    1024*4,
                    service,
                    4,
                    NULL) != pdPASS) 
    {
        ...
        DEBUG_LOGE(SD_PLAYER_CTRL_SER_TAG, "ERROR creating _task_sd_player_ctrl task! Out of memory?");
    }
	*/
...
void sd_music_start()
{
	...
	switch (folder_index)
	{
		...
		{
			player_mdware_play_tone(FLASH_MUSIC_D_BAIKE);
			break;
		}
		case FOLDER_INDEX_ERGE:
		{
			player_mdware_play_tone(FLASH_MUSIC_D_ERGE);
			break;
		}
		case FOLDER_INDEX_GUOXUE:
		{
			player_mdware_play_tone(FLASH_MUSIC_D_GUOXUE);
			break;
		}
		case FOLDER_INDEX_GUSHI:
		{
			player_mdware_play_tone(FLASH_MUSIC_D_GUSHI);
			break;
		}
		case FOLDER_INDEX_KETANG:
		{
			player_mdware_play_tone(FLASH_MUSIC_D_KETANG);
			break;
		}
		case FOLDER_INDEX_QINZI:
		{
			player_mdware_play_tone(FLASH_MUSIC_D_QINZI);
			break;
		}
		case FOLDER_INDEX_XIAZAI:
		{
			player_mdware_play_tone(FLASH_MUSIC_D_XIAZAI);
			break;
		}
		case FOLDER_INDEX_ENGLISH:
		{
			player_mdware_play_tone(FLASH_MUSIC_D_YINGYU);
			break;
		}
		case FOLDER_INDEX_MUSIC:
		{
			player_mdware_play_tone(FLASH_MUSIC_D_YINYUE);
			break;
		}
		default:
			break;
