...
void prevent_child_trigger()
{
	...
	if (prevent_child_trigger_switch == PREVENT_CHILD_TRIGGER_OFF)
	{
		...
		player_mdware_play_tone(FLASH_MUSIC_B_10_CHILD_LOCK_HAS_BEEN_OPENED);
//		EspAudioTonePlay(tone_uri[TONE_TYPE_10_CHILD_LOCK_HAS_BEEN_OPENED], TERMINATION_TYPE_NOW);
		vTaskDelay(2000);
	...
	else
	{
		...
		player_mdware_play_tone(FLASH_MUSIC_B_11_CHILD_LOCK_HAS_BEEN_CLOSED);
//		EspAudioTonePlay(tone_uri[TONE_TYPE_11_CHILD_LOCK_HAS_BEEN_CLOSED], TERMINATION_TYPE_NOW);
		vTaskDelay(2000);
