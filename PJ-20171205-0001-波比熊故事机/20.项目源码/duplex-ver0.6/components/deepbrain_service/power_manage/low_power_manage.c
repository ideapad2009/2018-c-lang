...
#define LOW_POWER_VOLTAGE		3.66				//播报低电量提示电压区间最高值
#define OFF_POWER_VOLTAGE		3.63				//播报关机提示电压
        ...
        if (xQueueReceive(xQueueLowPowerManage, &battery_vol, portMAX_DELAY) == pdPASS)
	    {
			...
			if(battery_vol <= LOW_POWER_VOLTAGE)
			{
				if (battery_vol <= OFF_POWER_VOLTAGE && play_low_first == 1)
				{
					...
					player_mdware_play_tone(FLASH_MUSIC_B_18_SHUTDOWN_PROMPTING);
					vTaskDelay(4*1000);
					deep_sleep_manage();
				}
				else
				{
					...
					{
						player_mdware_play_tone(FLASH_MUSIC_B_16_PLEASE_CHARGE_THE_BATTERY);
						vTaskDelay(3*1000);
						play_time = time(NULL);
						play_low_first = 1;
					}
...
int check_charging_state(void)
{
	int led_level = gpio_get_level(BATTERY_POWER_SCOUTING_GPIO);
