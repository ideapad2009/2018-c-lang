...
int cgiRedirectToHostname(HttpdConnData *connData) {
    ...
    buff=EspAudioAlloc(1, strlen((char *)connData->cgiArg) + sizeof(hostFmt));
    if (buff==NULL) {
        //Bail out
        return HTTPD_CGI_DONE;
    }
...
void httpdFlushSendBuffer(HttpdConnData *conn) {
    ...
    if (conn->priv->sendBuffLen!=0) {
        ...
        if (!r) {
            ...
            HttpSendBacklogItem *i=EspAudioAlloc(1, sizeof(HttpSendBacklogItem) + conn->priv->sendBuffLen);
            if (i==NULL) {
...
void httpdContinue(HttpdConnData * conn) {
    ...
    sendBuff=EspAudioAlloc(1, HTTPD_MAX_SENDBUFF_LEN);
    if (sendBuff==NULL) {
...
static void httpdParseHeader(char *h, HttpdConnData *conn) {
    ...
    if (strncmp(h, "SUBSCRIBE ", 10)==0) {
        ...
        firstLine=1;
    } else if (strncmp(h, "NOTIFY ", 7)==0) {
        conn->requestType = HTTPD_METHOD_NOTIFY;
        firstLine=1;
    ...
    if (firstLine) {
        ...
        *e=0; //terminate url part
        ...
        conn->post->buff=(char*)EspAudioAlloc(1, conn->post->buffSize + 1);
        if (conn->post->buff==NULL) {
            ESP_LOGE(HTTPD_TAG, "Malloc failed line(%d)", __LINE__);
            return;
        }
        conn->post->buffLen=0;
...
void httpdConnSendStart(HttpdConnData *conn) {
    httpdPlatLock();
    char *sendBuff=EspAudioAlloc(1, HTTPD_MAX_SENDBUFF_LEN);
    if (sendBuff==NULL) {
        ESP_LOGE(HTTPD_TAG, "Malloc failed line(%d)", __LINE__);
        return;
    }
...
int httpdConnectCb(ConnTypePtr conn, char *remIp, int remPort) {
    ...
    connData[i]=EspAudioAlloc(1, sizeof(HttpdConnData));
    if (connData[i]==NULL) {
        ESP_LOGE(HTTPD_TAG, "Malloc failed line(%d)", __LINE__);
        httpdPlatUnlock();
        return 0;
    ...
    connData[i]->priv=EspAudioAlloc(1, sizeof(HttpdPriv));
    memset(connData[i]->priv, 0, sizeof(HttpdPriv));
    connData[i]->conn=conn;
    connData[i]->slot=i;
    connData[i]->priv->headPos=0;
    connData[i]->post=EspAudioAlloc(1, sizeof(HttpdPostData));
