...
sock_t sock_connect(const char * const host, const char * const port)
{
    ...
    if (err != 0)
    {
    	printf("sock_connect getaddrinfo error:[%d]\r\n", err);  
        return INVALID_SOCK;
    }
	...
	const size_t ms)
{
	...
	do 
	{
		...
		if (ret == -1) 
		{
			printf("sock_readn_with_timeout select failed(%d)\r\n", errno);
			break;
		} 
		else if (ret == 0) 
		{
	...
	const sock_t sock, const void * const buff, const size_t len, const size_t ms)
{
	...
	while(data_len > 0)
	{
		...
		if (ret == -1) 
		{
			printf("sock_writen_with_timeout select failed(%d)\r\n", errno);
			break;
		} 
		else if (ret == 0) 
		{
...
int sock_get_errno(const sock_t sock)
{
	...
	if (ret != 0 || optval != 0)
	{
		printf("getsockopt ret[%d], sock errno = %d\r\n", ret, optval);
	}
