...
{
    ESP_LOGI(KEYCTRL_TAG, "key_ctrl cb: tl %d, state: %d\n", key_cb.tl, key_cb.state);
    switch (key_cb.state) {
    ...
    case KEY_ACT_STATE_RELEASE:
        ...
        break;
    default:
        ESP_LOGI(KEYCTRL_TAG, "Invalid key_ctrl state: %d\n", key_cb.state);
        break;
    }
}
...
{
    if (key_cb.key_tmr) {
        ESP_LOGW(KEYCTRL_TAG, "%s timer not released\n", __func__);
    ...
    if (key_cb.key_tmr == NULL) {
        ESP_LOGW(KEYCTRL_TAG, "%s timer creation failure\n", __func__);
        return false;
    }
    return true;
}
...
static void key_act_state_hdl_press(key_act_cb_t *key_cb, key_act_param_t *param)
{
    ...
    if (param->evt == ESP_AVRC_CT_PASSTHROUGH_RSP_EVT) {
        if (key_cb->tl != param->tl || key_cb->key_code != param->key_code
            ...
            ESP_LOGW(KEYCTRL_TAG, "Key pressed hdlr: invalid state\n");
            return;
        }
