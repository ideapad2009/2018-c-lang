...
//		led_ctrl_set_mode(LED_CTRL_EYE_LIGHT);
		player_mdware_play_tone(FLASH_MUSIC_A_02_PROMPT_NETWORK_CONNECTION_SUCCESS);
		set_wifi_state(WIFI_ON_STATE);
		g_first_connected = 1;
...
//				led_ctrl_set_mode(LED_CTRL_EYE_FLASHING_SLOW);
				...
				{
					player_mdware_play_tone(FLASH_MUSIC_A_06_PROMPT_NETWORK_DISCONNECTION);
    ...
    case SYSTEM_EVENT_STA_GOT_IP: {
        if (lastState != SYSTEM_EVENT_STA_GOT_IP) 
		{
            ...
            xQueueSend(xQueueWifi, &g_WifiState, 0);
			//EspAudioPlay(tone_uri[TONE_TYPE_DYY_WIFI_SUCCESS]);
...
static void WifiConfigTask(void *pvParameters)
{
    ...
    if ((ESP_OK == esp_wifi_get_mode(&curMode)) && (curMode == WIFI_MODE_STA)) {
        ...
        case ESP_ERR_TIMEOUT:
			...
			player_mdware_play_tone(FLASH_MUSIC_A_04_PROMPT_NETWORKING_TIMEOUT);
            break;
        case ESP_FAIL:
            ESP_LOGI(WIFICONFIG_TAG, "Smartconfig failed, please try again");
			player_mdware_play_tone(FLASH_MUSIC_A_03_PROMPT_NETWORK_CONNECTION_FAILURE);
            break;
        default:
            break;
        }
