...
typedef struct
{
	...
	int recv_len;
	char recv_buff[1024*10];
	char recv_buff_swap[1024*10];
	char asr_result[512];
...
static SINOVOICE_RECV_STATUS_T get_recv_result_status(void)
{
	if (g_recv_result_handle != NULL)
	{
		return g_recv_result_handle->recv_status;
	}

	return SINOVOICE_RECV_STATUS_STOP;
}
...
static void get_asr_result(char* str_reply, const size_t str_reply_len)
{
	if (g_recv_result_handle != NULL)
	{
		snprintf(str_reply, str_reply_len, "%s", g_recv_result_handle->asr_result);
	}
}
	...
	while (1)
	{
		...
		switch (handle->recv_status)
		{
			...
			case SINOVOICE_RECV_STATUS_START:
			{
				...
				memset(handle->asr_result, 0, sizeof(handle->asr_result));
				break;
			}
			case SINOVOICE_RECV_STATUS_RECVING:
			{
				...
				ret = sock_readn_with_timeout(
					handle->sock, handle->recv_buff + handle->recv_len, sizeof(handle->recv_buff) - handle->recv_len - 1, 200);
				...
				while(handle->recv_len > 0)
				{
					//find http head start
					char *http_start = strstr(handle->recv_buff, "HTTP/1.1");
					if (http_start == NULL)
					{
						break;
					}
					...
					if (http_get_error_code(handle->recv_buff) == 200)
					{
						...
						if (http_body != NULL)
						{
							...
							if (body_len == content_len)
							{
								...
								ESP_LOGE(TAG_ASR, "1[%s]", handle->recv_buff_swap);
								handle->recv_len = 0;
								if (strstr(handle->recv_buff_swap, "Success"))
								{
									char *p_start = strstr(handle->recv_buff_swap, "\"Text\":\"");
									if (p_start != NULL)
									{
										p_start += strlen("\"Text\":\"");
										...
										if (p_end != NULL)
										{
											memcpy(handle->asr_result, p_start, p_end-p_start);
											handle->asr_result[p_end-p_start] = '\0';
										}
									}
									handle->recv_status = SINOVOICE_RECV_STATUS_STOP;
								}
								break;
							}
							else if (body_len > content_len)
							{
								memset(handle->recv_buff_swap, 0, sizeof(handle->recv_buff_swap));
								memcpy(handle->recv_buff_swap, http_body, content_len);
								ESP_LOGE(TAG_ASR, "2[%s]", handle->recv_buff_swap);
	...
	SINOVOICE_ASR_HANDLE_T *_handler, const char *data, const uint32_t data_len)
{
	...
	else
	{
		if (data_len > 0)
		{
			...
			return SINOVOICE_NOT_REACH_16K;
		}
	}
...
SINOVOICE_ASR_ERRNO_T sinovoice_asr_write(void *_handler, const char *data, const uint32_t data_len)
{
	...
	{
		if (ret == SINOVOICE_NOT_REACH_16K)
		{
			return SINOVOICE_ASR_OK;
		}
		return ret;
	...
	DEBUG_LOGE(TAG_ASR, "[%d][%d]", http_buff->send_len, ret);
...
{	
	...
	for (i = 0; i < 10; i++)
	{
		if (get_recv_result_status() == SINOVOICE_RECV_STATUS_STOP)
		{
			get_asr_result(str_reply, str_reply_len);
			break;
		}
		vTaskDelay(200);
	}
