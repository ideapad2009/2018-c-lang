...
{
    int msg_len;
    while (mqtt_rb_available(&client->send_rb) < client->mqtt_state.outbound_message->length) {
        xQueueReceive(client->xSendingQueue, &msg_len, 1000 / portTICK_RATE_MS);
        mqtt_rb_read(&client->send_rb, client->mqtt_state.out_buffer, msg_len);
    }
    mqtt_rb_write(&client->send_rb,
    ...
    while (connected) {
        if (xQueueReceive(client->xSendingQueue, &msg_len, 1000 / portTICK_RATE_MS)) {
            ...
            while (msg_len > 0) {
                ...
                mqtt_rb_read(&client->send_rb, client->mqtt_state.out_buffer, send_len);
...
mqtt_client *mqtt_start(mqtt_settings *settings)
{
    ...
    mqtt_rb_buf = (uint8_t*) malloc(CONFIG_MQTT_QUEUE_BUFFER_SIZE_WORD * 4);
    ...
    mqtt_rb_init(&client->send_rb, mqtt_rb_buf, CONFIG_MQTT_QUEUE_BUFFER_SIZE_WORD * 4, 1);
