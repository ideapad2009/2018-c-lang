...
int get_memo_result(char *str_body)
{
	...
	if (NULL != pJson_body) 
	{
		...
		{
			ESP_LOGE(MEMO_TAG, "str_body_buf,pJson_cmd_data not found\n");
		...
		//备忘时间
		cJSON *pJson_array_4 = cJSON_GetArrayItem(pJson_array,4);
		if (NULL == pJson_array_4) 
		{
			ESP_LOGE(MEMO_TAG, "str_body_buf,pJson_array_4 not found\n");
			goto get_memo_result_error;
		}
		
		cJSON *pJson_attrValue_4 = cJSON_GetObjectItem(pJson_array_4, "attrValue");
		if (NULL == pJson_attrValue_4) 
		{
			ESP_LOGE(MEMO_TAG, "str_body_buf,pJson_attrValue_4 not found\n");
			goto get_memo_result_error;
		}

		//日期过期检查
		if(strncmp(pJson_attrValue_4->valuestring, "outDateError", 12) == 0) 
		{
			//失效原因
			cJSON *pJson_array_5 = cJSON_GetArrayItem(pJson_array,5);
			if (NULL == pJson_array_5) 
			{
				ESP_LOGE(MEMO_TAG, "str_body_buf,pJson_array_5 not found\n");
				goto get_memo_result_error;
			}
		
			cJSON *pJson_attrValue_5 = cJSON_GetObjectItem(pJson_array_5, "attrValue");
			if (NULL == pJson_attrValue_5) 
			{
				ESP_LOGE(MEMO_TAG, "str_body_buf,pJson_attrValue_5 not found\n");
				goto get_memo_result_error;
			}
			play_tts(pJson_attrValue_5->valuestring);
			goto get_memo_result_error;
		} 
		else 
		{
			//将特定格式的字符时间转换为struct tm格式
			strptime(pJson_attrValue_4->valuestring, "%Y,%m,%d,%H,%M,%S", &tm);
			time = mktime(&tm) - 28800;
			ESP_LOGE(MEMO_TAG, "**********time = [%ld]", time);
		}

		//备忘信息
		cJSON *pJson_array_1 = cJSON_GetArrayItem(pJson_array,1);
		if (NULL == pJson_array_1) 
		{
			ESP_LOGE(MEMO_TAG, "str_body_buf,pJson_array_1 not found\n");
			goto get_memo_result_error;
		}
		
		cJSON *pJson_attrValue_1 = cJSON_GetObjectItem(pJson_array_1, "attrValue");
		if (NULL == pJson_attrValue_1) 
		{
			ESP_LOGE(MEMO_TAG, "str_body_buf,pJson_attrValue_1 not found\n");
			goto get_memo_result_error;
		}
		ESP_LOGE(MEMO_TAG, "pJson_attrValue_1 = %s", pJson_attrValue_1->valuestring);

		cJSON *pJson_attrName_1 = cJSON_GetObjectItem(pJson_array_1, "attrName");
		if (NULL == pJson_attrName_1) 
		{
			ESP_LOGE(MEMO_TAG, "str_body_buf,pJson_attrName_1 not found\n");
			goto get_memo_result_error;
		}
		ESP_LOGE(MEMO_TAG, "pJson_attrName_1 = %s", pJson_attrName_1->valuestring);

		//备忘提示语
		cJSON *pJson_array_12 = cJSON_GetArrayItem(pJson_array,12);
		if (NULL == pJson_array_12) 
		{
			ESP_LOGE(MEMO_TAG, "str_body_buf,pJson_array_12 not found\n");
			goto get_memo_result_error;
		}
		
		cJSON *pJson_attrValue_12 = cJSON_GetObjectItem(pJson_array_12, "attrValue");
		if (NULL == pJson_attrValue_12) 
		{
			ESP_LOGE(MEMO_TAG, "str_body_buf,pJson_attrValue_12 not found\n");
			goto get_memo_result_error;
		}

		memo_struct_handle->memo_event.time = time;
		snprintf(memo_struct_handle->memo_event.str, sizeof(memo_struct_handle->memo_event.str), pJson_attrValue_12->valuestring);
		memo_struct_handle->memo_status = MEMO_ADD;

		char *str = (char *)esp32_malloc(64 * sizeof(char));
		if (NULL == str)
		{
			ESP_LOGE(MEMO_TAG, "str esp32_malloc fail\n");
		}
		else
		{
			memset(str, 0, 64);
			strncat(str, pJson_attrName_1->valuestring, 12);
			strncat(str, pJson_attrValue_1->valuestring, 52);
			play_tts(str);
			esp32_free(str);
			str = NULL;
		}	
