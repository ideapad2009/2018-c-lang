...
void event_callback(APP_OBJECT_t *app, APP_EVENT_MSG_t *msg)
{
	DEBUG_LOGE(DEEPBRAIN_SER_TAG, "event[%08x]", msg->event);
	switch (msg->event)
	{
		default:
			break;
...
void task_app(void *p)
{
	...
	if (app == NULL)
	{
		DEBUG_LOGE(DEEPBRAIN_SER_TAG, "new app failed, out of memory");
	}
	else
	{
		app_set_loop_timeout(app, 1000, event_callback);
		app_add_event(app, APP_EVENT_DEFAULT_BASE, event_callback);
		app_add_event(app, APP_EVENT_WIFI_BASE, event_callback);
		app_add_event(app, APP_EVENT_SDCARD_BASE, event_callback);
		app_add_event(app, APP_EVENT_POWER_BASE, event_callback);
		DEBUG_LOGE(DEEPBRAIN_SER_TAG, "new app success");
	}
...
void task_send_msg(void *p)
{	int i = 0;

	while(1)
	{
		APP_EVENT_MSG_t msg = {"app", "app", APP_EVENT_DEFAULT_BASE, "123456", strlen("123456")};
		app_send_msg(&msg);
		device_sleep(0, 1000*100);
	}
}
...
static void DeepBrainActive(MediaService *self)
{
	...
	if (xTaskCreate(task_app,
                    ...
                    NULL) != pdPASS) 
   	{
        ...
        ESP_LOGE(DEEPBRAIN_SER_TAG, "ERROR creating task_app task! Out of memory?");
    }

	if (xTaskCreate(task_send_msg,
                    "task_send_msg",
                    1024*2,
                    NULL,
                    10,
                    NULL) != pdPASS) 
   	{
        ...
        ESP_LOGE(DEEPBRAIN_SER_TAG, "ERROR creating task_send_msg task! Out of memory?");
    }
