...
#if 0
static void volume_control_process(cJSON *json_data, MediaService* media)
{
	int i = 0;
	int array_size = 0;
	char *attr_name_vol_ctrl = "音量控制";
	char *attr_Name_vol_graduation = "音量调节刻度";

	cJSON *pJson_cmd_attrs = cJSON_GetObjectItem(json_data, "commandAttrs");
	if (NULL != pJson_cmd_attrs)
	{
		...
		if (array_size > 0)
		{
			...
			int volume_num = 6;
			int vol_now = 40;
			if (NULL != media->getVolume)
			{	
				vol_now = media->getVolume((MediaService *) media);
				DEBUG_LOGE(DEEPBRAIN_SER_TAG, "old vol_now : %d\n", vol_now);
			}
			for (i = 0; i < array_size; i++)
			{
				cJSON *pJson_cmd_attrs_first = cJSON_GetArrayItem(pJson_cmd_attrs, i);
				cJSON *pJson_attr_name = cJSON_GetObjectItem(pJson_cmd_attrs_first, "attrName");
				if (strcmp(pJson_attr_name->valuestring, attr_name_vol_ctrl) == 0)
				{
					cJSON *pJson_attr_value_type = cJSON_GetObjectItem(pJson_cmd_attrs_first, "attrValue");
					if (NULL != pJson_attr_value_type && NULL != pJson_attr_value_type->valuestring)
					{		
						if (strcmp(pJson_attr_value_type->valuestring, "调高") == 0)
						{
							volume_type = EM_VOLUME_HIGHER;
						}
						if (strcmp(pJson_attr_value_type->valuestring, "调低") == 0)
						{
							volume_type = EM_VOLUME_LOWER;
						}
						if (strcmp(pJson_attr_value_type->valuestring, "适中") == 0)
						{
							volume_type = EM_VOLUME_MODERATE;
						}
						if (strcmp(pJson_attr_value_type->valuestring, "最大") == 0)
						{
							volume_type = EM_VOLUME_MAX;
						}
						if (strcmp(pJson_attr_value_type->valuestring, "最小") == 0)
						{
							volume_type = EM_VOLUME_MIN;
						}			
					}
					else
					{
						DEBUG_LOGE(DEEPBRAIN_SER_TAG, "pJson_attr_value_type wrong\n");	
					}			
				}
				else if (strcmp(pJson_attr_name->valuestring, attr_Name_vol_graduation) == 0)
				{
					cJSON *pJson_attr_value_graduation = cJSON_GetObjectItem(pJson_cmd_attrs_first, "attrValue");
					if (NULL != pJson_attr_value_graduation && NULL != pJson_attr_value_graduation->valuestring)
					{
						if (strcmp(pJson_attr_value_graduation->valuestring, "") != 0)
						{
							volume_num = atoi(pJson_attr_value_graduation->valuestring);
							if (volume_num >= 0 && volume_num <= 100)
							{
								volume_num = 60 * volume_num / 100;
							}
						}
					}
				}	
			}
			if (NULL != media->setVolume)
			{	
				switch (volume_type)
				{
					case EM_VOLUME_HIGHER:
					{
						volume_num += vol_now;
						if (volume_num > 100)
						{
							media->setVolume((MediaService *) media, 0, 100);
							play_tts(media, "已经最大了哦");
						}
						else
						{
							media->setVolume((MediaService *) media, 0, volume_num);
							play_tts(media, "已经调大了");
						}
						if (NULL != media->getVolume)
						{	
							vol_now = media->getVolume((MediaService *) media);
							DEBUG_LOGE(DEEPBRAIN_SER_TAG, "now vol_now : %d\n", vol_now);
						}
						break;
					}
					case EM_VOLUME_LOWER:
					{
						volume_num = vol_now - volume_num;
						if (vol_now < 40)
							{
								media->setVolume((MediaService *) media, 0, 40);
								play_tts(media, "已经最小了哦");
							}
						else
						{
							media->setVolume((MediaService *) media, 0, volume_num);
							play_tts(media, "已经调小了");
						}
						if (NULL != media->getVolume)
						{	
							vol_now = media->getVolume((MediaService *) media);
					...
					case EM_VOLUME_MODERATE:
					{
						if (NULL != media->setVolume)
						{
							if ((media->setVolume((MediaService *) media, 0, 55) < 0)) 
							{
								 DEBUG_LOGE(DEEPBRAIN_SER_TAG, "Cannot set volume now");
							}
							play_tts(media, "已经调好了");
							if (NULL != media->getVolume)
							{	
								vol_now = media->getVolume((MediaService *) media);
								DEBUG_LOGE(DEEPBRAIN_SER_TAG, "now vol_now : %d\n", vol_now);
					...
					case EM_VOLUME_MAX:
					{
						if (NULL != media->setVolume)
						{
							if ((media->setVolume((MediaService *) media, 0, 100) < 0)) 
							{
								 DEBUG_LOGE(DEEPBRAIN_SER_TAG, "Cannot set volume now");
							}
							play_tts(media, "音量已最大");
							if (NULL != media->getVolume)
							{	
								vol_now = media->getVolume((MediaService *) media);
								DEBUG_LOGE(DEEPBRAIN_SER_TAG, "now vol_now : %d\n", vol_now);
					...
					case EM_VOLUME_MIN:
					{
						if (NULL != media->setVolume)
						{
							if ((media->setVolume((MediaService *) media, 0, 40) < 0)) 
							{
								 DEBUG_LOGE(DEEPBRAIN_SER_TAG, "Cannot set volume now");
							}
							play_tts(media, "音量已最小");
							if (NULL != media->getVolume)
							{	
								vol_now = media->getVolume((MediaService *) media);
								DEBUG_LOGE(DEEPBRAIN_SER_TAG, "now vol_now : %d\n", vol_now);
					...
					default:
					{
						play_tts(media, DEEPBRAIN_ERROR); 
						break;
					}
				}
			}
