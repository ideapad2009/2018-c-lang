...
static bool bt_app_post_msg(bt_app_msg_t *msg)
{
    ...
    if (xQueueSend(m_tsk_que, msg, 10 / portTICK_RATE_MS) != pdTRUE) {
        ESP_LOGE(BT_TAG, "App msg post failed\n");
        return false;
    }
    return true;
}
...
static void bt_app_context_switched(bt_app_msg_t *msg)
{
    ESP_LOGD(BT_TAG, "App context switched\n");
    if (msg->cb) {
...
static void m_tsk_que_handler(bt_app_msg_t *msg)
{
    ...
    ESP_LOGD(BT_TAG, "App hdl evt: sig 0x%x, 0x%x\n", msg->sig, msg->event);
    switch (msg->sig) {
    case BT_APP_SIG_CONTEXT_SWITCH:
        bt_app_context_switched(msg);
        break;
    default:
        ESP_LOGW(BT_TAG, "Invalid app event: sig (0x%x)\n", msg->sig);
        break;
    } // switch (msg->sig)
        ...
        if (act_que == m_tsk_que) {
            ...
            for (int i = 0; i < BT_APP_MAX_EXT_QUE_NUM; i++) {
                ...
                if (p_item->xque == NULL) {
                    ...
                    ESP_LOGD(BT_TAG, "App msg received from ext_que\n");
