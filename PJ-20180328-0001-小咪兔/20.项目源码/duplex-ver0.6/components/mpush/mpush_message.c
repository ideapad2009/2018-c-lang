...
{
	ESP_LOGI(PRINT_TAG, "push msg:%s", _body);
	...
	if (NULL != pJson) 
	{
		...
		{
			ESP_LOGE(PRINT_TAG, "msgType not found");
			goto _mpush_decode_push_msg_error;
		}
		...
		else if (strncasecmp(pJson_msg_type->valuestring, "file", 4) == 0)
		{
	...
	{
		ESP_LOGE(PRINT_TAG, "invalid json content[%s]", _body);
	}
...
int _mpush_decode_ok_msg(char *_body, size_t _body_len)
{
	...
	printf("cmd=%d,code=%d,data len=%u,%s\r\n", cmd, code, len, msg_buf);
