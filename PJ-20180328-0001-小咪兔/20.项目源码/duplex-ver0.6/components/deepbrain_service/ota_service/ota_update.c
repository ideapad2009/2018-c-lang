	...
	size_t *_out_firmware_len)
{
	...
	sock_set_write_timeout(sock, 2, 0);

	snprintf(header, sizeof(header), GET_OTA_REQUEST_HEADER, params, domain);
	if (sock_write(sock, header, strlen(header)) != strlen(header)) 
	{
    ...
    while (1) 
	{
        ...
        int buff_len = sock_read(sock, str_buf, sizeof(str_buf) - 1);
	...
	size_t _out_len)
{
	...
	if (sock_write(sock, str_buf, strlen(str_buf)) != strlen(str_buf)) 
	{
		ESP_LOGE(OTA_TAG, "sock_write http header fail");
		goto ota_update_check_fw_version_error;
	}
	...
	memset(str_buf, 0, sizeof(str_buf));
	sock_readn(sock, str_buf, sizeof(str_buf) - 1);
	sock_close(sock);
	...
	else
	{
		...
		//ESP_LOGE(OTA_TAG, "str_body is %s", str_buf);
		if (str_body == NULL)
		{
			goto ota_update_check_fw_version_error;
		}
