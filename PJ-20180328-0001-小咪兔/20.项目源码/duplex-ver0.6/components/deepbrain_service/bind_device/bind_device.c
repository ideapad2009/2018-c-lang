...
{  
	...
	char str_buf[BUFFER_SIZE_128] = {0};
	int listenfd = -1;
	int connectfd = -1;
	...
	while(!g_bind_device_quit)
	{
		...
		{		
			...
			if (recv_len > sizeof(str_buf) - 1)
			{
				DEBUG_LOGE(BIND_TAG, "recv_len:[%d] too long, max size = %d", recv_len, sizeof(str_buf) -1);
				goto _task_process_device_bind_error;
			}
			
			int ret = sock_readn_with_timeout(connectfd, str_buf, recv_len, 2000);
			if (ret == recv_len)
			{
				DEBUG_LOGE(BIND_TAG, "recv info:[%d]%s", ret, str_buf);
				save_user_id(str_buf);
				
				char device_id[32]= {0};
				get_flash_cfg(FLASH_CFG_DEVICE_ID, &device_id);
				snprintf(str_buf, sizeof(str_buf), DEVICE_BIND_INFO, device_id, ESP32_FW_VERSION);
				DEBUG_LOGE(BIND_TAG, "write info:[%d]%s", strlen(str_buf), str_buf);
				recv_len = htonl(strlen(str_buf));
				...
				if (sock_writen_with_timeout(connectfd, str_buf, strlen(str_buf), 2000) != strlen(str_buf))
				{
					DEBUG_LOGE(BIND_TAG, "sock_write body %d bytes fail", strlen(str_buf));
				}
			}
			else
			{
