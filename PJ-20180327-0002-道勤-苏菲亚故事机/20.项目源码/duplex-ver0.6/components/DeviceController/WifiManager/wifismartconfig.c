...
static void smartconfig_done(smartconfig_status_t status, void *pdata)
{
    ...
    case SC_STATUS_GETTING_SSID_PSWD:
		...
		set_wifi_start_link_flag();	//记录为开始连接，记号设为1
		get_connect_wifi_time();//获取开始联网时间点
...
esp_err_t WifiSmartConnect(uint32_t ticks_to_wait)
{
    ...
    // WiFi connect timeout
        ...
        //esp_wifi_stop();删除原因：调用esp_wifi_stop()会导致SD卡播不出来，原因不明
...
void set_event_group_stop_bits()
{
	xEventGroupSetBits(g_sc_event_group, SC_STOP_REQ_EVT);
}
