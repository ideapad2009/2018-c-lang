...
static void bt_app_a2d_connect(void)
{
    ...
    ESP_LOGI(BT_TAG, "Reconnect [%02x:%02x:%02x:%02x:%02x:%02x]\n", p[0], p[1], p[2], p[3], p[4], p[5]);
...
static void bt_app_handle_evt(uint16_t event, void *p_param)
{
    ESP_LOGD(BT_TAG, "bt_app_handle_evt 0x%x\n", event);
    ...
    case ESP_A2D_CONNECTION_STATE_EVT: {
        ...
        ESP_LOGI(BT_TAG, "A2DP connection state changed: state %d\n", a2d->conn_stat.state);
    ...
    case ESP_A2D_AUDIO_STATE_EVT: {
        a2d = (esp_a2d_cb_param_t *)(p_param);
        ESP_LOGI(BT_TAG, "A2DP audio state changed: state %d\n", a2d->audio_stat.state);
    ...
    case ESP_A2D_AUDIO_CFG_EVT: {
        a2d = (esp_a2d_cb_param_t *)(p_param);
        ESP_LOGI(BT_TAG, "A2DP audio configured: type %d\n", a2d->audio_cfg.mcc.type);
    ...
    default:
        ESP_LOGI(BT_TAG, "Invalid A2DP event: 0x%x\n", event);
        break;
    }
...
{
    ESP_LOGD(BT_TAG, "Handle AVRC event: 0x%x\n", event);
    ...
    case ESP_AVRC_CT_PASSTHROUGH_RSP_EVT: {
        if (p_bt_av->avrc_connected) {
            ESP_LOGI(BT_TAG, "AVRC passthroun cmd rsp: key_code 0x%x, key_state %d\n", rc->psth_rsp.key_code, rc->psth_rsp.key_state);
    ...
    case ESP_AVRC_CT_KEY_STATE_CHG_EVT: {
        if (p_bt_av->avrc_connected) {
            ...
            ESP_LOGI(BT_TAG, "AVRC send key id0x%x, state %d\n", key_s->key_code, key_s->key_state);
            key_act_param_t param;
            memset(&param, 0, sizeof(key_act_param_t));
            param.evt = event;
    ...
    case ESP_AVRC_CT_PT_RSP_TO_EVT: {
        ESP_LOGI(BT_TAG, "AVRC passthrough cmd rsp time out\n");
        if (p_bt_av->avrc_connected) {
            key_act_param_t param;
            memset(&param, 0, sizeof(key_act_param_t));
            param.evt = event;
    ...
    case ESP_AVRC_CT_REMOTE_FEATURES_EVT: {
        ...
        ESP_LOGI(BT_TAG, "AVRC remote features: 0x%x, [%02x:%02x:%02x:%02x:%02x:%02x]\n",
                 rc->rmt_feats.feat_mask, bda[0], bda[1], bda[2], bda[3], bda[4], bda[5]);
        break;
    }
#endif //IDF_3_0
    default:
        ESP_LOGI(BT_TAG, "Invalid AVRC event: 0x%x\n", event);
        break;
    }
}
...
static void bt_app_handle_rec_evt(uint16_t event, void *p_param)
{
    ...
    default:
        ESP_LOGE(BT_TAG, "Invalid reconnection event: 0x%x\n", event);
        break;
    }
}
...
void deviceEvtNotifiedToBt(DeviceNotification *event)
{
    ...
    ESP_LOGI(BT_TAG, "deviceEvtNotifiedToBt type%x", evt->type);
    EspAudioPrintMemory(BT_TAG);
    if (event && (event->type == DEVICE_NOTIFY_TYPE_TOUCH)) {
        ...
        case DEVICE_NOTIFY_KEY_PLAY:
            if (playFlag) {
                ...
                ESP_LOGI(BT_TAG, "Play");
            } else {
                avrcpTransfer(ESP_AVRC_PT_CMD_PAUSE);
                ESP_LOGI(BT_TAG, "Pause");
            }
            playFlag = !playFlag;
            break;
        default:
...
static void player_status_updated(void *param)
{
    ...
    ESP_LOGI(BT_TAG, "Player status updated: status 0x%x, err 0x%x\n", p->status, p->errMsg);
...
{
    ESP_LOGI(BT_TAG, "bluetoothControlActive, freemem %d", esp_get_free_heap_size());
...
{
    ESP_LOGI(BT_TAG, "bluetoothControlStop\r\n");
}

BluetoothControlService *BluetoothControlCreate()
{
    ESP_LOGI(BT_TAG, "BluetoothControlCreate\r\n");
