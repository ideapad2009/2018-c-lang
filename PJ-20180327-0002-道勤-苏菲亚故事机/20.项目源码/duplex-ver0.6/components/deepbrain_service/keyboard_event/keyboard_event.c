...
#define KEY_LOG(a,b,c,d,e) printf("Keyevent===========[value-%d] [key-%d] [event-%d] [%s-%dms]\n", a, b, c, d, e)

int adc_keyboard_process(KEYBOARD_OBJ_T *kobj, int adc_value)
{	
	...
	for (int i = 0; i <= kobj->k_cfg.key_num; i++)//查找匹配的按键
	{
		...
		{
			KEY_LOG(adc_value, 0, 0, "NULL", 0);//没有匹配值
			return 0;
		}
	}
...
KEYBOARD_OBJ_T *keyboard_create(int key_num)
{
	if (key_num > KEY_NUM) {
		return NULL;
	}
	...
	kobj = (KEYBOARD_OBJ_T *)esp32_malloc(sizeof(KEYBOARD_OBJ_T));	
	if (kobj == NULL) {
		return NULL;
	}
	...
	kobj->k_cfg.key_num        = key_num;
	kobj->k_cfg.fault_tol      = 100;
	kobj->k_cfg.scan_cycle     = 40;
	kobj->k_cfg.longpress_time = 800;
