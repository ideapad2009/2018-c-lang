...
//#define SMARTCONFIG_TIMEOUT_TICK            (36000 * 1000 / portTICK_RATE_MS)//10 hours
#define SMARTCONFIG_TIMEOUT_TICK            (60 * 1000 / portTICK_RATE_MS)//60s
...
static esp_err_t wifiEvtHandlerCb(void *ctx, system_event_t *evt)
{
	...
	*/
    ...
    case SYSTEM_EVENT_STA_DISCONNECTED:
        ...
        //set_wifi_state_flag(WIFI_STATE_DISCONNECT);//网络已断开记号
		if (get_wifi_mode_state() != WIFI_MODE_OFF)
        {
            if (lastState != SYSTEM_EVENT_STA_DISCONNECTED) 
			{
				if (g_first_connected == 1)//网络至少连接上一次才执行以下操作
				{
					...
					if (get_wifi_mode_state() != WIFI_MODE_OFF)
					{
						//led_ctrl_set_mode(LED_CTRL_U_D_EYE_ALTERANTE_SCINTILLATION);//无网络上下眼交替闪烁,此位置避免下方延迟导致指示灯延迟开启
						uart_esp32_status_mode(UART_ESP32_STATUS_MODE_WIFI_CONNECTING);
						if (g_wifi_mode_handle->wifi_flag.play_disconnect_tone == 1)
						{
							if (g_mic_flag == 0)
							{
    ...
    case SYSTEM_EVENT_STA_GOT_IP: {
        if (lastState != SYSTEM_EVENT_STA_GOT_IP) 
		{
			...
			auto_play_stop();
			vTaskDelay(200);
			uart_esp32_status_mode(UART_ESP32_STATUS_MODE_WIFI_CONNECTED);
			if (g_mic_flag == 0)
			{
				player_mdware_play_tone(FLASH_MUSIC_NETWORK_CONNEC_SUCCESS);
			}
			g_wifi_mode_handle->wifi_flag.play_disconnect_tone = 1;
        }
...
static int WifiConfigActive(DeviceManager *self)
{
	...
	vTaskDelay(4*1000);
	uart_esp32_status_mode(UART_ESP32_STATUS_MODE_WIFI_SETTING);
	if (g_mic_flag == 0)
	{
...
void wifi_smart_connect()
{
    ...
    esp_wifi_disconnect();
	g_wifi_mode_handle->wifi_flag.play_disconnect_tone = 0;		//设为0，不可以播放
		...
		switch (res) 
		{
	        ...
	        case ESP_FAIL:
	        {
	        	if (get_wifi_mode_state() != WIFI_MODE_OFF)	
	        	{
					...
					if (g_mic_flag == 0)
					{
						player_mdware_play_tone(FLASH_MUSIC_NETWORK_CONNEC_OUT_TIME);
					}
	        	}
	            break;
	        }
...
void wifi_mode_config_create()
{
	...
	g_wifi_mode_handle->wifi_flag.play_disconnect_tone = 1;		//初始为1可以播放
...
void connect_wifi_error_manage()
{
	if (g_wifi_mode_handle->wifi_flag.wifi_start_link_flag == 1)	//判断是否已进入连接状态，1为已进入连接
	{
		...
		if ((current - g_wifi_mode_handle->wifi_flag.connect_wifi_time) > 60*1000)//计算WiFi连接时长，做超时计算，单位毫秒
		{
