...
/*new code*****************************************************************************************************************/
static void translate_process(TRANSLATE_MSG_T *p_msg)
{
	int ret = 0;
	...
	switch (p_msg->asr_result.type)
	{
		...
		{
			if (strlen(p_msg->asr_result.asr_text) > 0)
			{
				translate_ret = translate_and_return_translate_result(p_msg->asr_result.asr_text, translate_result_text);
			...
			ESP_LOGI(TRANSLATE_TAG, "asr:[%s],result:[%s]", p_msg->asr_result.asr_text, translate_result_text);
			break;
		}
		default:
			break;
	...
	int id, char *data, size_t data_len, int record_ms, int is_max)
{
	...
	p_msg->record_data.record_type = ASR_RECORD_TYPE_AMRWB;
	p_msg->call_back = translate_asr_result_cb;
