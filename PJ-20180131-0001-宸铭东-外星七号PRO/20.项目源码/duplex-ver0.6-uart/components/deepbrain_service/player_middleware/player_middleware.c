				...
				case PLAYER_MDWARE_MSG_RECORD_START:
				{
					...
					for (i=0; i<MAX_REC_SHARE_NUM; i++)
					{
						...
						if (p_msg->msg_start_recrod.cb == rec_info->call_back)
						{
							...
							rec_info->call_back(rec_info->record_index*(-1), NULL, 0, rec_info->record_total_ms, 0);
							rec_info->record_id = p_msg->msg_start_recrod.record_id;
				...
				if (len > 0)
				{
					for (i=0; i<MAX_REC_SHARE_NUM; i++)
					{
						...
						if (rec_info->call_back != NULL)
						{
							...
							{
								rec_info->call_back(rec_info->record_index, data_share_stream->rec_data, len, rec_info->record_total_ms, 1);
								rec_info->record_index++;
								rec_info->call_back(rec_info->record_index*(-1), NULL, 0, rec_info->record_total_ms, 0);
								memset(rec_info, 0 ,sizeof(REC_SHARE_INFO_T));
							}
							...
							{
								rec_info->call_back(rec_info->record_index, data_share_stream->rec_data, len, rec_info->record_total_ms, 0);
							}
						}
					}
				}
