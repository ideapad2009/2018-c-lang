...
#define TTS_URL           "http://cdnmusic.hezi.360iii.net/prod/tts"
#define BAIDU_TTS         "http://tsn.baidu.com/text2audio"
...
void save_function()
{
	...
	memset(music_info, 0, sizeof(MusicInfo));
	if (EspAudioInfoGet(music_info) != ESP_OK || music_info->name == NULL)
	{
		...
		goto save_function_error;
	}
	else
	{
		ESP_LOGE("MUSIC URL", "URL= [%s]", music_info->name);
		if (strncmp (music_info->name, TTS_URL, sizeof(TTS_URL)-1) == 0 || 
			strncmp (music_info->name, BAIDU_TTS, sizeof(BAIDU_TTS)-1) == 0)
		{
			player_mdware_play_tone(FLASH_MUSIC_B_03_CAN_NOT_BE_COLLECTED_AT_PRESENT);
			goto save_function_error;
		}
	    ...
	    if (strcmp (g_music_url, "") != 0 && strncmp (g_music_url, "http", 4) == 0)
	    {
	    	if (exclude_repetition_url() == EXCLUDE_RET_NOT_EXIST)
	    	{
			    ...
			    {
			    	player_mdware_play_tone(FLASH_MUSIC_B_17_ADD_TO_THE_COLLECTION_LIST_FAIL);
			    }
	    	}
			else
			{
...
int init_collection_function()
{
	...
	memset((char*)g_music_url, 0, URL_SIZE);
	...
	memset((char*)g_music_url_info, 0, URL_SIZE);
