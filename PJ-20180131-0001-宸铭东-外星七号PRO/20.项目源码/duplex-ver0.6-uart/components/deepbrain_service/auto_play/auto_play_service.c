...
static void task_auto_play(void *pv)
{
    ...
    while (1) 
	{
			...
			switch (msg)
			{
				...
				case AUTO_PLAY_MSG_PLAY_PAUSE:
				{
					...
					{			
						...
						{
							uart_esp32_status_mode(UART_ESP32_STATUS_MODE_MUSIC_STOP);//向MCU发送停止状态
							EspAudioPause();
		...
		if (player_status.status == AUDIO_STATUS_ERROR)
		{
			...
			{
				player_mdware_play_tone(FLASH_MUSIC_MUSIC_NOT_FOUND);
				auto_play_stop();
			}
