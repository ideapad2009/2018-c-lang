...
static void adc_key_matrix_run(xTimerHandle tmr)
{
	...
	if (esp_efuse_vref_is_cali() == 2) 
	{
		adc_value = get_adc_voltage_tp(ADC_ATTEN_11db, ADC1_CHANNEL_3);
#ifdef PRINT_ADC_VALUE
        printf("channel %d,TWO Point adc:%d\r\n", ADC1_CHANNEL_3, adc_value);
#endif
    } 
	else if (esp_efuse_vref_is_cali() == 1) 
	{
		adc_value = get_adc_voltage_deepbrain(ADC_ATTEN_11db, ADC1_CHANNEL_3);
#ifdef PRINT_ADC_VALUE
        printf("channel %d,Vref adc:%d\r\n", ADC1_CHANNEL_3, adc_value);
#endif
	} 
	...
	{
		adc_value = adc1_get_voltage(ADC1_CHANNEL_3);
#ifdef PRINT_ADC_VALUE
        printf("channel %d,No calibration data, Raw:%d\r\n", ADC1_CHANNEL_3, adc_value);
#endif
	}

	key_event = adc_keyboard_process(g_keyboard_obj, adc_value);

	if (key_event != 0)
	{
		key_matrix_callback(key_event);
	}
...
{
	g_keyboard_obj = keyboard_create(5);
	...
	KEYBOARD_CFG_T keyboard_cfg = {
		...
		.keyboard[5] = {//微聊键
			...
			.longpress   = KEY_EVENT_WECHAT_START,
			.release     = KEY_EVENT_WECHAT_STOP,
		},
	};

	g_keyboard_obj->k_cfg = keyboard_cfg;
}
