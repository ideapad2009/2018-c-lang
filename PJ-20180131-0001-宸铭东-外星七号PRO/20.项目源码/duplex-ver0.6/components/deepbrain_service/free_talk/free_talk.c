...
void free_talk_nlp_result_cb(NLP_RESULT_T *result)
{
	...
	switch (result->type)
	{
		...
		case NLP_RESULT_TYPE_CHAT:
		{
			...
			else if (strcmp(&result->input_text, "当前版本") == 0)
			{
				if (free_talk_get_state() != FREE_TALK_STATE_TRANSLATE_CH_EN)
				{
					//free_talk_autostart_enable();
				...
				{
					//free_talk_autostart_enable();
					nlp_service_send_translate_request(&result->chat_result.text, free_talk_nlp_result_cb);
				}
			}
			else if(strcmp(&result->input_text, "中译英") == 0)
			{
				if (free_talk_get_state() != FREE_TALK_STATE_TRANSLATE_CH_EN)
				{
					//free_talk_autostart_enable();
			...
			else if(strcmp(&result->input_text, "英译中") == 0)
			{
				//free_talk_autostart_enable();
...
static void free_talk_asr_result_cb(ASR_RESULT_T *result)
{
	...
	switch (result->type)
	{
		...
		case ASR_RESULT_TYPE_SUCCESS:
		{
			set_talk_tone_state(FREE_TALK_TONE_STATE_ENABLE_PLAY);
			free_talk_autostart_enable();
			if(strcmp(result->asr_text, "英译中") == 0)
			{
				free_talk_set_state(FREE_TALK_STATE_TRANSLATE_EN_CH);
				player_mdware_play_tone(FLASH_MUSIC_ENGLISH_CHINESE_TRANSLATE);
			}
...
#if SPEECH_CTRL_TRANSLATE_MODEL
	g_free_talk_handle->talk_tone_switch = FREE_TALK_TONE_STATE_ENABLE_PLAY;
#endif	
