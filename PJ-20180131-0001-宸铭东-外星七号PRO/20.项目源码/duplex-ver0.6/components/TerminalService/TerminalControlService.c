...
static void play_geling_edu(void *ref, int argc, char *argv[])
{
    TerminalControlService *term = (TerminalControlService *)ref;
    char *string;
    int index = 0;
    ...
    if (argc > 0) 
	{
        ESP_LOGI(TERM_TAG, "playedu %s", argv[0]);
        string = argv[0];
		geling_edu_service_send_request(string);
    } 
	else 
	{
        ESP_LOGE(TERM_TAG, "Invalid param, Support CMD: playedu %s", TEST_M3U8_LINK);
        return;
    }
}
...
static void set_asr_mode(void *ref, int argc, char *argv[])
{
    ...
    int index = 0;
    int termType = 0;
	ASR_ENGINE_TYPE_t mode = ASR_ENGINE_TYPE_AMRNB;
    ...
    if (argc > 0) 
	{
        if (strcmp(argv[0], "0") == 0)
        {
        	...
        	mode = ASR_ENGINE_TYPE_AMRNB;
			set_flash_cfg(FLASH_CFG_ASR_MODE, &mode);
		}
		else if (strcmp(argv[0], "1") == 0)
		{
			ESP_LOGE(TERM_TAG, "asrmode %s", argv[0]);
        	mode = ASR_ENGINE_TYPE_AMRWB;
			set_flash_cfg(FLASH_CFG_ASR_MODE, &mode);
		}
		else
		{
...
static void set_asr_url(void *ref, int argc, char *argv[])
{
    TerminalControlService *term = (TerminalControlService *)ref;
    char *string;
    int index = 0;
    ...
    if (argc > 0) 
	{
        if (strlen(argv[0]) > 0)
        {
        	ESP_LOGE(TERM_TAG, "asrurl %s", argv[0]);
        	deep_brain_asr_set_url(argv[0]);
		}
		else
		{
			ESP_LOGE(TERM_TAG, "Invalid param, Support CMD: asrurl http://asr.deepbrain.ai/open-api/asr/recognise");
		}
    } 
	else 
	{
