...
static void task_ota_update(void *pvParameters)
{
	...
	while(1)
	{
		...
		//set_extern_keyword_wakeup_permit(EXTERN_KEYWORD_WAKEUP_STATE_PERMIT_DISABLE);//ota start,wakeup disable
		keyword_wakeup_stop();//wakeup stop
		free_talk_stop();
		auto_play_stop();
		...
		else
		{
			...
			keyword_wakeup_start();//wakeup start
			//set_extern_keyword_wakeup_permit(EXTERN_KEYWORD_WAKEUP_STATE_PERMIT_ENABLE);//ota stop,wakeup enable
