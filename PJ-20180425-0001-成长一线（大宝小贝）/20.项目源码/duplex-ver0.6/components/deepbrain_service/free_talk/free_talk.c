...
void free_talk_nlp_result_cb(NLP_RESULT_T *result)
{
	...
	switch (result->type)
	{
		...
		{
			asr_no_result_count++;
			printf("************ in NLP_RESULT_TYPE_ERROR 1 , asr_no_result_count = [%d]\n", asr_no_result_count);
			if (asr_no_result_count >= FREE_TALK_MAX_ASR_NO_RET_COUNT)
			{
				printf("************ in NLP_RESULT_TYPE_ERROR 2 , asr_no_result_count = [%d]\n", asr_no_result_count);
		...
		{
			asr_no_result_count++;
			printf("************ in NLP_RESULT_TYPE_NO_ASR 1 , asr_no_result_count = [%d]\n", asr_no_result_count);
			if (asr_no_result_count >= FREE_TALK_MAX_ASR_NO_RET_COUNT)
			{
				printf("************ in NLP_RESULT_TYPE_NO_ASR 2 , asr_no_result_count = [%d]\n", asr_no_result_count);
				free_talk_autostart_disable();
				asr_no_result_count = 0;
				set_free_talk_status(FREE_TALK_RUN_STATUS_STOP);
			}
...
static void free_talk_asr_result_cb(ASR_RESULT_T *result)
{
	...
	switch (result->type)
	{
		...
		case ASR_RESULT_TYPE_FAIL:
		{
			set_talk_tone_state(FREE_TALK_TONE_STATE_DISABLE_PLAY);
			asr_fail_count++;
			printf("************ in ASR_RESULT_TYPE_FAIL 1 , asr_fail_count = [%d]\n", asr_fail_count);
			if (asr_fail_count >= FREE_TALK_MAX_ASR_FAIL_COUNT)
			{
				printf("************ in ASR_RESULT_TYPE_FAIL 2 , asr_no_result_count = [%d]\n", asr_no_result_count);
		...
		case ASR_RESULT_TYPE_NO_RESULT:
		{
			set_talk_tone_state(FREE_TALK_TONE_STATE_DISABLE_PLAY);
			asr_no_result_count++;
			printf("************ in ASR_RESULT_TYPE_NO_RESULT 1 , asr_no_result_count = [%d]\n", asr_no_result_count);
			if (asr_no_result_count >= FREE_TALK_MAX_ASR_NO_RET_COUNT)
			{
				printf("************ in ASR_RESULT_TYPE_NO_RESULT 2 , asr_no_result_count = [%d]\n", asr_no_result_count);
				free_talk_autostart_disable();
				asr_no_result_count = 0;
				set_free_talk_status(FREE_TALK_RUN_STATUS_STOP);
				//led_ctrl_set_mode(LED_CTRL_EYE_LIGHT);
	...
	FREE_TALK_HANDLE_T *free_talk_handle, FREE_TALK_RECORD_DATA_T *p_record_data, int is_last)
{
	...
	switch (free_talk_get_state())
	{
		...
		case FREE_TALK_STATE_TRANSLATE_EN_OR_CH:
		{
			p_msg->record_data.language_type = ASR_LANGUAGE_TYPE_CHINESE;
