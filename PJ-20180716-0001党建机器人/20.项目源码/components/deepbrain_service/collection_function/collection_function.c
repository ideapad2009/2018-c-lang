...
#define LOG_TAG "COLLECTION"

COLLECTION_URL_T *music_url = NULL;
...
{	
	printf("in get_save_index_num 1!\n");
	nvs_handle my_handle;
	esp_err_t err;
	
	err = nvs_open(COLLECTION_PARAMS_NAMESPACE, NVS_READWRITE, &my_handle);
	printf("in get_save_index_num 2!\n");
	if (err != ESP_OK) 
	{
...
{	
	printf("in get_play_index_num 1!\n");
	nvs_handle my_handle;
	esp_err_t err;
	
	err = nvs_open(COLLECTION_PARAMS_NAMESPACE, NVS_READWRITE, &my_handle);
	printf("in get_play_index_num 2!\n");
	if (err != ESP_OK) 
	{
...
void save_function()
{
	MusicInfo *music_info = esp32_malloc(sizeof(MusicInfo));
	if (EspAudioInfoGet(music_info) != ESP_OK)
	{
		esp32_free(music_info);
		ESP_LOGE(LOG_TAG, "EspAudioInfoGet music_info fail!");
	...
	else
	{
		...
		esp32_free(music_info);
	    if (strncmp (g_music_url, "http", 4) == 0)
	    {
	    	if (exclude_repetition_url() == EXCLUDE_RET_NOT_EXIST)
	    	{
				if (save_url() == ESP_OK)
			    {
					...
					save_save_url_index();
					led_mcu_cmd(LED_CMD_NUM_NET_SUCCESS);
					vTaskDelay(1500);
					led_mcu_cmd(LED_CMD_NUM_WAIT_4);
					return ;
			    }
	    	}
	    }
	...
	else
	{
		...
		while (1)
		{
			...
			if (latest_play_index_num == _latest_play_index_num)
			{
				if (get_url() == ESP_OK)
				{
					printf("in get_prev_music 6\n");
					snprintf(src->music_url, sizeof(src->music_url), "%s", g_music_url_info);
					save_play_url_index();
				}
				else
				{
					printf("in get_prev_music 7\n");
					vTaskDelay(3000);
...
static void get_next_music(AUTO_PLAY_MUSIC_SOURCE_T *src)
{
	...
	if (latest_play_index_num > PLAY_INDEX_NUM_TWENTY)
	{
		printf("in get_next_music 2\n");
		latest_play_index_num = PLAY_INDEX_NUM_ONE;
		printf("000latest_play_index_num not change! latest_play_index_num = %d\n", latest_play_index_num);
	}
	if (get_url() == ESP_OK)
	{
		printf("in get_next_music 3\n");
		snprintf(src->music_url, sizeof(src->music_url), "%s", g_music_url_info);
		save_play_url_index();
	}
	else
	{
		printf("in get_next_music 4\n");
		PLAY_INDEX_NUM _latest_play_index_num = latest_play_index_num;
		while (1)
		{
			latest_play_index_num++;
			...
			if (latest_play_index_num == _latest_play_index_num)
			{
				if (get_url() == ESP_OK)
				{
					printf("in get_next_music 6\n");
					snprintf(src->music_url, sizeof(src->music_url), "%s", g_music_url_info);
					save_play_url_index();
				}
				else
				{
					printf("in get_next_music 7\n");
					vTaskDelay(3000);
					//player_mdware_play_tone(FLASH_MUSIC_B_04_THE_COLLECTION_LIST_IS_EMPTY);
//					EspAudioPlay(tone_uri[TONE_TYPE_04_THE_COLLECTION_LIST_IS_EMPTY]);
					vTaskDelay(3000);
