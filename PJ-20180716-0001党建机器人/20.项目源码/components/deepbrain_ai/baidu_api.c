...
static const char *BAIDU_TTS_URL = "http://tsn.baidu.com/text2audio"
	"?tex=%s&lan=zh&cuid=%s&ctp=1&tok=%s&per=4&spd=4";
	...
	char *str_tts_text)
{
	...
	device_get_mac_str(str_mac, sizeof(str_mac));
	len = snprintf(str_url, str_url_len, BAIDU_TTS_URL, str_buf, str_mac, g_str_token);
	esp32_free(str_buf);
