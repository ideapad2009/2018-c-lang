...
static void func_name_decode(cJSON *pJson_func_name, cJSON *pJson_cmd_name)
{
	
	if(strstr(pJson_cmd_name->valuestring, "聊天") != NULL)
	{
		printf("*************** 聊天\n");
		uart_esp32_status_mode(UART_ESP32_STATUS_MODE_CHAT);//向MCU发送ESP32聊天状态
	}
	else if (strstr(pJson_func_name->valuestring, "音乐") != NULL 
		|| strstr(pJson_func_name->valuestring, "歌曲") != NULL 
		|| strstr(pJson_func_name->valuestring, "英文歌") != NULL 
		|| strstr(pJson_func_name->valuestring, "儿歌") != NULL)
	{
		printf("*************** 音乐\n");
		uart_esp32_status_mode(UART_ESP32_STATUS_MODE_MUSIC);//向MCU发送播放ESP32音乐状态
	}
	else if(strstr(pJson_func_name->valuestring, "常识") != NULL 
		|| strstr(pJson_func_name->valuestring, "为什么") != NULL)
	{
		printf("*************** 常识\n");
		uart_esp32_status_mode(UART_ESP32_STATUS_MODE_KNOWLEDGE);//向MCU发送ESP32常识状态
	}
	else if(strstr(pJson_func_name->valuestring, "英语") != NULL 
		|| strstr(pJson_func_name->valuestring, "英文") != NULL)
	{
		printf("*************** 英语\n");
		uart_esp32_status_mode(UART_ESP32_STATUS_MODE_ENGLISH);//向MCU发送ESP32英语状态
	}
	else if(strstr(pJson_func_name->valuestring, "计算器") != NULL)
	{
		printf("*************** 计算器\n");
		uart_esp32_status_mode(UART_ESP32_STATUS_MODE_ARITHMETIC);//向MCU发送ESP32计算器状态
	}
	else if(strstr(pJson_func_name->valuestring, "天气") != NULL)
	{
		printf("*************** 天气\n");
		uart_esp32_status_mode(UART_ESP32_STATUS_MODE_WEATHER);//向MCU发送ESP32天气状态
	}
	else
	{
		printf("*************** 其它\n");
		uart_esp32_status_mode(UART_ESP32_STATUS_MODE_OTHERS);//向MCU发送ESP32其它状态
	}
}
...
NLP_DECODE_ERRNO_t dp_nlp_result_decode(
...
{	
    ...
    if (NULL != pJson_body) 
	{
		...
		//检测播放内容
		cJSON *pJson_func_name = cJSON_GetObjectItem(pJson_cmd_data, "functionName");
		if (pJson_func_name == NULL)
		{
			goto dp_nlp_result_decode_error;
		}
		...
		//解析功能名称，向MCU发送相应状态
		func_name_decode(pJson_func_name, pJson_cmd_name);
		...
		{
			get_memo_result(json_string);
			nlp_result->type = NLP_RESULT_TYPE_CMD;
		}
		else
		{
			if (music_source_decode(pJson_cmd_data, nlp_result) != NLP_DECODE_ERRNO_OK)
			{
				goto dp_nlp_result_decode_error;
			}
		}		
